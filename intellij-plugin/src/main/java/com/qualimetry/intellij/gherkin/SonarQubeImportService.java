...
final class SonarQubeImportService {
    ...
    private String resolveProfileKey(String profileNameOrKey) throws IOException, InterruptedException {
        ...
        if (!input.isEmpty()) {
            for (int i = 0; i < keys.length; i++) {
                ...
                if (k != null && k.equals(profileNameOrKey.trim())) {
                    return k;
                }
                if (k != null && k.toLowerCase().contains(input)) {
                    return k;
                }
                if (n != null && n.toLowerCase().equals(input)) {
                    return k;
                }
                if (n != null && n.toLowerCase().contains(input)) {
                    return k;
                }
            }
        }
    ...
    private void extractRuleKeysFromRulesResponse(String body, Map<String, GherkinAnalyzerSettings.RuleOverride> out) {
        int i = 0;
        while ((i = body.indexOf(REPO_PREFIX, i)) >= 0) {
            ...
            if (end > start) {
                ...
                if (!ruleKey.isEmpty()) {
                    out.put(ruleKey, new GherkinAnalyzerSettings.RuleOverride(true, null));
                }
            }
