...
/**
 * Settings panel for the Qualimetry Gherkin Analyzer plugin.
 * Provides per-rule enable/disable, severity override, and filter.
 */
final class GherkinAnalyzerSettingsPanel {
    ...
    GherkinAnalyzerSettingsPanel() {
        enabledCheckBox = new JCheckBox("Enable Qualimetry Gherkin Analyzer");
        enabledCheckBox.setSelected(true);

        autoSyncCheckBox = new JCheckBox("Automatically sync rules from SonarQube on startup");
        autoSyncCheckBox.setSelected(true);
        autoSyncCheckBox.setToolTipText("Re-imports rules from the last-used SonarQube server and profile when the IDE starts");
        ...
        JButton importButton = new JButton("Import from SonarQube...");
        importButton.setToolTipText("Import active rules from a SonarQube quality profile");
        importButton.addActionListener(e -> ImportFromSonarQubeAction.runImport(null, () ->
                loadFrom(GherkinAnalyzerSettings.getInstance())));

        JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 0));
        buttonPanel.add(importButton);
        buttonPanel.add(resetButton);

        defaultRuleKeys = Set.copyOf(CheckList.getDefaultRuleKeys());
    ...
    private static String keyToDescription(String key, Rule rule) {
    ...
    private List<RuleRow> buildRuleRows() {
        ...
        for (Class<? extends BaseCheck> clazz : CheckList.getAllChecks()) {
            ...
            String key = rule.key();
            String description = keyToDescription(key, rule);
    ...
    private void applyFilter(String filter) {
        ...
        for (RuleRow r : visible) {
            GherkinAnalyzerSettings.RuleOverride override = settings.rules.get(r.key);
    ...
    boolean isAutoSyncOnStartup() {
        return autoSyncCheckBox.isSelected();
    }

    void setAutoSyncOnStartup(boolean value) {
        autoSyncCheckBox.setSelected(value);
    }
    ...
    void loadFrom(@NotNull GherkinAnalyzerSettings settings) {
        enabledCheckBox.setSelected(settings.enabled);
        autoSyncCheckBox.setSelected(settings.autoSyncOnStartup);
        rulesReplaceDefaultsCheckBox.setSelected(settings.rulesReplaceDefaults);
        applyFilter(filterField.getText());
    }

    void saveTo(@NotNull GherkinAnalyzerSettings settings) {
        settings.enabled = enabledCheckBox.isSelected();
        settings.autoSyncOnStartup = autoSyncCheckBox.isSelected();
        ...
        Map<String, GherkinAnalyzerSettings.RuleOverride> existingRules = new java.util.HashMap<>(settings.rules);
        Map<String, GherkinAnalyzerSettings.RuleOverride> rules = settings.rules;
        rules.clear();
        if (settings.rulesReplaceDefaults) {
            for (RuleRow r : allRows) {
                RowState state = tableState.get(r.key);
                GherkinAnalyzerSettings.RuleOverride existing = existingRules.get(r.key);
                boolean enabled;
                String severity;
                if (state != null) {
                ...
                rules.put(r.key, withExistingParams(new GherkinAnalyzerSettings.RuleOverride(enabled, severity), existing));
            }
        } else {
            for (RuleRow r : allRows) {
                RowState state = tableState.get(r.key);
                ...
                boolean hasParams = existing != null && existing.params != null && !existing.params.isEmpty();
                boolean differsFromDefault = (enabled != r.inDefault) || (severity != null) || hasParams;
    ...
    private static GherkinAnalyzerSettings.RuleOverride withExistingParams(
            GherkinAnalyzerSettings.RuleOverride next,
            GherkinAnalyzerSettings.RuleOverride existing) {
        if (existing != null && existing.params != null && !existing.params.isEmpty()) {
            next.params.putAll(existing.params);
        }
        return next;
    }
