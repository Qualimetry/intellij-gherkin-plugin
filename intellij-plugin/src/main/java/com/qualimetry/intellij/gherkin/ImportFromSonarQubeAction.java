...
/**
 * Imports active Gherkin rules from a SonarQube quality profile into the plugin settings.
 */
public final class ImportFromSonarQubeAction extends AnAction {

    @Override
    public void actionPerformed(@NotNull AnActionEvent e) {
        runImport(e.getProject(), null);
    }

    static void runImport(@Nullable Project project, @Nullable Runnable onImported) {
        ...
        if (serverUrl.isEmpty()) {
            showNotification(project, "Server URL is required.", NotificationType.ERROR);
            return;
        }
        String enteredToken = dialog.getToken();
        ...
        ProgressManager.getInstance().run(new Task.Backgroundable(project, "Importing from SonarQube", true) {
            ...
            public void run(@NotNull ProgressIndicator indicator) {
                try {
                    ...
                    if (token.isEmpty()) {
                        String saved = SonarQubeTokenStore.load();
                        if (saved != null) {
                            token = saved.trim();
                        }
                    }
                    ...
                    if (!enteredToken.isEmpty()) {
                        SonarQubeTokenStore.store(enteredToken);
                    }
                    ApplicationManager.getApplication().invokeLater(() -> {
                        if (count == 0) {
                            showNotification(project,
                                    "The selected profile has no active rules; nothing was imported.",
    ...
    private static void showNotification(Project project, String content, NotificationType type) {
        NotificationGroupManager.getInstance()
                .getNotificationGroup("Qualimetry Gherkin")
