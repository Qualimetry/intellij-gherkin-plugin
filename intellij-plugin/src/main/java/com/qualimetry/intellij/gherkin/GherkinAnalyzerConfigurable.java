...
/**
 * Settings UI under Settings &gt; Tools &gt; Qualimetry Gherkin Analyzer.
 */
public final class GherkinAnalyzerConfigurable implements Configurable {

    private GherkinAnalyzerSettingsPanel panel;

    @Nls(capitalization = Nls.Capitalization.Title)
    @Override
    public String getDisplayName() {
        return "Qualimetry Gherkin Analyzer";
    }

    @Nullable
    @Override
    public JComponent createComponent() {
        panel = new GherkinAnalyzerSettingsPanel();
    ...
    public boolean isModified() {
        ...
        if (panel.isAutoSyncOnStartup() != settings.autoSyncOnStartup) return true;
        if (panel.isRulesReplaceDefaults() != settings.rulesReplaceDefaults) return true;
        GherkinAnalyzerSettings temp = new GherkinAnalyzerSettings();
