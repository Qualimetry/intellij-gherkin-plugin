...
- **Rule descriptions** - a tool window explains why a rule exists, how to fix the finding, and related rules; open it from a finding's quick fixes.
- **Default quality profile** - 55 rules active out of the box for immediate value.
...
Enter your server URL, an optional authentication token, and an optional profile name or key. When no profile is given, the server's default Gherkin profile is used. The imported rules replace the current configuration. The server URL and profile name are remembered between sessions, and the token (if provided) is stored securely in the IDE's credential store.
